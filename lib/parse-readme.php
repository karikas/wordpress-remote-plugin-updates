<?php
// From https://github.com/markjaquith/WordPress-Plugin-Readme-Parser/blob/master/parse-readme.php
// Modified to not be reliant on WordPress functions being present
// and a couple other minor tweaks, including major screenshot overhaul
...
Class WordPress_Readme_Parser {
	...
	function parse_readme_contents( $file_contents ) {
		...
		if ( preg_match( '|Donate link:(.*)|i', $file_contents, $_donate_link ) ) {
			$donate_link = trim($_donate_link[1]);
		} else {
			$donate_link = null;
		}
		...
		// MKARIKAS - adding code to only grab short description from last line, if multiple lines are returned here
		if ( strpos( $short_desc_filtered, "\n" ) !== false ) {
			$short_desc_parts    = explode( "\n", $short_desc_filtered );
			$short_desc_filtered = trim( array_pop( $short_desc_parts ) );
		}
		...
		foreach (
			...
			if ( isset( $sections[ $special_section ] ) ) {
				$final_sections[ str_replace('frequently_asked_questions', 'faq', $special_section) ] = $sections[ $special_section ]['content'];
		...
		if ( isset( $final_sections['screenshots'] ) ) {
			...
			if ( $screenshots ) {
				foreach ( (array) $screenshots as $num => $ss ) {
					// Check if there's a png or jpg in the assets folder to match
					// If so, continue and take note of the extension
					if (file_exists(__DIR__.DIRECTORY_SEPARATOR.'..'.DIRECTORY_SEPARATOR.'assets'.DIRECTORY_SEPARATOR.'screenshot-'.($num+1).'.png')) {
						$ext = 'png';
					} elseif (file_exists(__DIR__.DIRECTORY_SEPARATOR.'..'.DIRECTORY_SEPARATOR.'assets'.DIRECTORY_SEPARATOR.'screenshot-'.($num+1).'.jpg')) {
						$ext = 'jpg';
					} else {
						continue 1;
					}
					$screenshot_html = "<a href=\"{$base_url}screenshot-".($num+1).".{$ext}\" target=\"_blank\"><img src=\"{$base_url}screenshot-".($num+1).".{$ext}\" /></a><p>".strip_tags($ss[1]).'</p>';
					$final_screenshots[] = $screenshot_html;
				}
			}
		}
		$final_sections['screenshots'] = '<ol><li>'.implode('</li><li>', $final_screenshots).'</li></ol>';
